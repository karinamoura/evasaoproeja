<x-admin>
    ...
    <div class="card">
        ...
        <form action="{{ route('admin.user.update',$user) }}" method="POST" class="needs-validation" novalidate>
            @method('PUT')
            @csrf
                            ...
                            <select name="role"
                                ...
                                @foreach ($roles as $role)
                                    <option value="{{ $role->name }}"
                                        {{ ($user->roles->count() > 0 && $user->roles[0]['name'] === $role->name) || old('role') === $role->name ? 'selected' : '' }}>
